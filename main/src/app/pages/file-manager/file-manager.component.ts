...
export class FileManagerComponent implements AfterViewInit {
  ...
  constructor() {
    ...
    // set icon size
    this.setIconSizeIndex(
      localStorage.getItem("icon-size") ?? this.default_icon_size_index
    ); // default is medium
  ...
  setUpSocket() {
    socket.startBackgroundProcess = (
      type: fs,
      data: any,
      progressIndefinite: boolean = true
    ) => {
      let process = {
  ...
  normalize_path(path: string): string {
    path = path
      .replace("://", ":")
      .replace(":/", ":")
      .replace(":", "://"); // if collon(:) is present then it should be ://
  ...
  loadDirContents(path: string, fromSplash: boolean = false) {
    ...
    fetch(`${domain}/fs/${protocol}/dir-contents/`, {
      ...
      .catch(err => {
        ...
        let err_msg =
          err.name === "customError" ? err.message : "Something went wrong";
  ...
  strcmp(a: any, b: any): 1 | -1 | 0 {
    let x = a.name.toLowerCase(),
      y = b.name.toLowerCase(); //ignore case
  ...
  setIconSizeIndex(index: any): void {
    this.iconSizeIndex = Math.min(
      this.iconSizes.length - 1,
      Math.max(0, index)
    );
  ...
  getConnections(): any[] {
    return (
      JSON.parse(atob(localStorage.getItem("connections") ?? "") || "[]") || []
    );
  }
  ...
  deleteConnection(event: any, index: number) {
    ...
    this.showPopUp(
      "confirm",
      "Delete Connection",
      `Are you sure you want to delete the connection : "${this.connections[index].name}"?`,
      "delete",
      "Delete",
      (data: any) => {
  ...
  testConnection(server: string, user: string, password: string) {
    ...
    fetch(`${domain}/fs/ssh/connect`, {
      ...
      .then(data => {
        if (data.status) {
          if (data.data == this.current_test_connection) {
            // if not it is from old request
      ...
      .catch(error => {
        if (testing_connection == this.current_test_connection) {
          // if not it is from old request
  ...
  showPopUp(
    ...
    cancel: string | null,
    cancelHandler: any,
    backgroundCancellation: boolean = true,
    misk: string | null = null,
    miskHandler: any = null
  ) {
    icon = AVAILABLE_POP_UP_ICONS.includes(icon ?? "default")
      ? icon
      : "default";
    this.popUp = {
      active: true,
  ...
  newFileFolder(type: string) {
    ...
    let pattern = isFolder
      ? /^New Folder [0-9]+$/
      : /^Text Document [0-9]+\.txt$/;
  ...
  getContentContainerStyle() {
    let classList = ["contents", "scroller"];
  ...
  isItemCutForPaste(index: number): boolean {
    ...
    return this.paste?.type != "cut-paste" ||
      this.paste?.source?.baseFolder != this.getCWD() ||
      this.paste?.source?.server != this.current_server
      ? false
      : this.paste?.files?.some((item: any) => {
  ...
  pasteIt() {
    ...
    let isValid = this.isValidPaste(
      paste_data.source.baseFolder,
      targetBaseFolder,
      paste_data.files
    );
    ...
    if (isValid.status == "confirm") {
      this.showPopUp(
        "confirm",
        "Paste Conflict",
        isValid.message || "Some Files/Folders already exists",
        "paste",
        "Skip",
        (data: any) => {
    ...
    this.showPopUp(
      ...
      (data: any) => {
        if (data != null) {
          if (data != item.name) {
            // not same name
            if (this.contents.some(existing => existing.name == data)) {
              // already another file/folder exists with same name
  ...
  openFileInDefaultApp(filename: any) {
    if (this.current_server != null) {
      ...
      this.toast(
        "warning",
        "You cannot open a file in server, copy it and paste it to your local system to open"
      );
    } else if (this.inBrowser) {
      this.toast(
        "warning",
        "Opening a file feature is not available in browser, try in the executable version"
      );
    } else {
