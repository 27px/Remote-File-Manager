...
function copyToPath(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (isFolder) {
        // create folder
        await target_promise.mkdir(target_path).catch(async (e) => {
          // check if failed due to already exixts error
          stat = await target_promise
            .stat(target_path)
...
module.exports = async (operation) => {
  ...
  if (typeof connection === "undefined" && server != null) {
    // either connected or local
  ...
  if (operation.type === "new-folder") {
    ...
    try {
      let queue = operation.data.files,
        baseFolder = operation.data.source.baseFolder;
      ...
      if (allSuccess) {
        ...
        // partial success
        send_success(
          "Some items deleted, but some items could not be deleted",
          true,
          "partial-success"
        );
      }
    ...
    if (typeof target_connection === "undefined" && target != null) {
      // either connected or local
      return send_error(null, "Not Connected to Server");
    }
    ...
    try {
      ...
      if (allSuccess) {
        ...
        // partial success
        send_success(
          "Some items copied, but some items could not be copied",
          true,
          "partial-success"
        );
      }
