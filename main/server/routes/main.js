...
} = require("../functions/response_functions.js");
...
route.post("/fs/ssh/connect", async (req, res) => {
  ...
  let ssh = null,
    ...
    reconnectTries = 1,
    reconnectDelay = 0,
    readyTimeout = 10000; // unique id used to identify server with user
  try {
...
route.post("/fs/:protocol/dir-contents", async (req, res) => {
  ...
  if (protocol === "ssh") {
    ...
    try {
      ...
      let stat = await Promise.allSettled(
        data.map(content => sftp.stat(getPath(content.filename, dir_path)))
      );
      stat = stat.map(stat_result =>
        stat_result.status == "fulfilled" ? stat_result.value : null
      );
      let filled = await Promise.allSettled(
        data.map((content, i) => {
          return data[i].longname[0] == "d"
            ? sftp.readdir(getPath(content.filename, dir_path))
            : false;
        })
      );
      let contents = data.map((content, i) => {
        ...
        let mime_type = !folder
          ? mime.getType(content.filename.split(".").pop())
          : null;
        return {
  ...
  } else if (isWin && dir_path === "/") {
    // local and windows and loading root path, so load drives
    nodeDiskInfo
      .getDiskInfo()
      .then(async disks => {
        ...
        let contents = disks.map((disk, i) => {
          ...
          let content = {
            // unlike dir contents permissions are not sent
    ...
    // not windows or not loading root path /
    try {
      ...
      let isDir = stat.map((item_stat, i) => {
        return item_stat == null
          ? !data[i].includes(".")
          : item_stat.isDirectory(); // stat is not available, probable permission issue, so check if "." is present to check file or not, does not work in different cases;
      });
      let filled = await Promise.allSettled(
        data.map((content, i) => {
